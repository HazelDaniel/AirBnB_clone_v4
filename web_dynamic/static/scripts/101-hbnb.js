$(document).ready(function () {
  ...
  $(document).on('change', ".amenities>.popover li>input[type='checkbox']", populateCheckedAmenities);
  $(document).on('change', ".locations>.popover li>input[type='checkbox']", populateCheckedLocations);
  $(document).on('change', ".locations>.popover li>ul>li>input[type='checkbox']", populateCheckedCities);
  ...
  $.ajax({
    ...
    success: function (data) {
      for (const place_entry of data) {
        $('.places ').append('<article><h2>' + place_entry.name + '</h2><div class="price_by_night"><p>$' + place_entry.price_by_night + '</p></div><div class="information"><div class="max_guest"><div class="guest_image"></div><p>' + place_entry.max_guest + '</p></div><div class="number_rooms"><div class="bed_image"></div><p>' + place_entry.number_rooms + '</p></div><div class="number_bathrooms"><div class="bath_image"></div><p>' + place_entry.number_bathrooms + '</p></div></div><div class="description"><p>' + place_entry.description + '</p></div></article>');
      }
    }
  });

  $('.filters > button').click(function () {
		console.log("checked cities\n", checkedCities, "\nchecked amenities\n", checkedAmenities, "\nchecked states\n", checkedStates);
